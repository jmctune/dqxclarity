...
def instantiate(exe):
  ...
  try:
    ...
    sys.exit(input(click.secho('Cannot find DQX. Ensure the game is launched and try again.\nIf you launched DQX as admin, this program must also run as admin.\n\nPress ENTER or close this window.', fg='red')))

def address_scan(handle: int, pattern: bytes, multiple: bool, *, index_pattern_list = [], startAddress = 0, endAddress = 0x7FFFFFFF):
  """
  Scans the entire virtual memory space for a handle and 
  returns addresses that match the given byte pattern.
  """
  next_region = startAddress
  while next_region < endAddress:
    next_region, found = pymem.pattern.scan_pattern_page(handle, next_region, pattern, return_multiple = multiple)
...
def jump_to_address(handle: int, address: int, pattern: str):
  ...
  This function exists as `scan_pattern_page` errors out when
  attempting to read protected pages, instead of just ignoring
  the page.
  """
...
def get_latest_from_weblate():
  ...
  try:
    ...
    sys.exit(click.secho('Failed to get latest files from weblate.\nMessage: {e}', fg='red'))
    
  try:
    shutil.rmtree('json/_lang/en/dqxclarity-weblate')
    shutil.rmtree('json/_lang/en/en')
  except:
    pass
  ...
  for file in archive.namelist():
    if file.startswith('dqxclarity-weblate/json/_lang/en'):
      ...
      shutil.move(f'json/_lang/en/{file}', f'json/_lang/en/{name[0]}{name[1]}')
  archive.close()
...
def translate():
  ...
  # Scans for all occurrences of indexPattern
  index_pattern_list = []
  address_scan(handle, indexPattern, True, index_pattern_list = index_pattern_list)
  ...
  with alive_bar(len(__flatten(index_pattern_list)), 
    ...
    for address in __flatten(index_pattern_list):
      ...
      # Read the first 64 bytes of the address and format it into hex ('DE AD BE EF')
      hex_result = __split_string_into_spaces(read_bytes(address, 64).hex().upper())
      
      # Lookup the found hex against the dataframe to find the file to apply
      csv_result = __flatten(df[df.hex_string == hex_result].values.tolist())

      if csv_result != []:
        
        # Parse just the filename from the dataframe
        file = os.path.splitext(os.path.basename(csv_result[0]))[0].strip()

        # Compare the cached md5 with the new. If different, rebuild
        # the hex file against the json. Otherwise, use the old hex file
        ...
        # We found the start of the indexPattern block, but now need to jump
        # to the beginning of what we want to replace the text with
        start_addr = jump_to_address(handle, address, textPattern)
        if start_addr:

          # Jump a few bytes ahead to get outside of the TEXT block
          start_addr = start_addr + 14
          
          # Iterate through memory one byte at a time until byte doesn't
          # equal 00. This puts us at the first byte where data starts. We
          # then backpedal one byte as the hex file has the very first word
          # prepended with 00
          result = type(byte)
          while True:
          ...
          # Read the hex file and convert it to a byte type (b'\x00),
          # then write
          data = bytes.fromhex(Path(f'hex/files/{file}.hex').read_text())
          pymem.memory.write_bytes(handle, start_addr, data, len(data))
  
  click.secho('Done. Continuing to scan for changes. Minimize this window and enjoy!', fg='green')       

def scan_for_ad_hoc_game_files():
  """
  Constantly scans the DQXGame process for known addresses
  ...
  for address in __flatten(index_pattern_list):
    ...
    if csv_result != []:
      ...
      # Parse the filename from the dataframe
      file = os.path.splitext(os.path.basename(csv_result[0]))[0].strip()
    ...
    for address in __flatten(index_pattern_list):
      ...
      if start_addr is not None: 
        ...
        if end_addr is not None:
          ...
          game_data = read_bytes(start_addr, bytes_to_read).hex()[24:].strip('00')
          ...
          csv_result = __flatten(df[df.hex_string == hex_result].values.tolist())
          if csv_result != []:
            file = os.path.splitext(os.path.basename(csv_result[0]))[0].strip() + '.json'
            ...
            __write_file('game_file_dumps', 'consider_master_dict.csv', 'a', f'json\\_lang\\en\\{file},{hex_result}\n')
...
def __flatten(list):
  """Takes a list of lists and __flattens it into one list."""
